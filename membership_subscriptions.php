...
$membership_subscriptions = new main_class;
...
$query_result = $membership_subscriptions->select($columns,$table,$condition);
...
<title>Club subscriptions</title>
<!-- InstanceEndEditable -->
...
<div id="club_members_wrapper" style="width: 60%; margin-left:20%; float:left;">
    ...
    <div id="program_heading" style="width: 40%; float:left;"><h3>Program</h3></div>
    <?php
    $club_id = $result['club_id'];
    $columns = "*";
    $table = "subscription";
    $condition = "club = $club_id  AND approval_status = 2";
    
    $member_query_result = $membership_subscriptions->select($columns,$table,$condition);
    echo "\n\n\n\n";
	if(mysql_num_rows($member_query_result) == 0)
	{
		echo "There are currently no pending subscriptions";
		$no_data = 1;
	}
    else 
	{
					...
					{?>
						<div align="justify" id="student_id" style="width: 20%; float:left;">
							<?php
								...
								$member_id = $member_result_set['member_id'];
							?>
						</div>
						...
						<div id="accept_subscription" style="width: 10%; float:left;">
							<?php
								echo "<a href=subscription_accept.php?user_id=";
								echo $student_id."&&club_id=";
								echo $club_id." title = 'Accept subscription'>";
								echo "Accept</a>";
							?>
						</div>
                        
                        <div id="reject_subscription" style="width: 10%; float:left;">
							<?php
								echo "<a href=subscription_reject.php?user_id=";
								echo $student_id."&&club_id=";
								echo $club_id." title = 'Reject subscription'>";
								echo "Reject</a>";
							?>
						</div>
		<?php		 }//END WHILE
